...
namespace TropicalServer.DAL
{
    public class ReportsDAL
    {
        ...
        public DataSet ValidateAccount(string UserID, string Password)
        {
            DataSet ds = new DataSet();
            ...
            try
            {
                ...
                {
                    connection.Open();
                    SqlCommand command = new SqlCommand($"select UserID, Password, EmailID FROM tblUserLogin WHERE UserID = \'{UserID}\' and Password = \'{Password}\'", connection);
                    // command.CommandType = CommandType.StoredProcedure;
                    //  command.CommandTimeout = 6000;
                    SqlDataAdapter adp = new SqlDataAdapter(command);
                    adp.Fill(ds);
        ...
        public DataSet GetAllOrders()
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    SqlCommand command = new SqlCommand(@"select TOP (100)  OrderID, OrderTrackingNumber, OrderDate, OrderCustomerNumber, CustName,
                                                                CustOfficeAddress1, OrderRouteNumber, OrderGroupNumber, OrderCaseNumbers, OrderItemNumber,
                                                                OrderPromoNumber, OrderItemQty, ItemUnitPrice, ItemTotalPrice, DeliveryDate, UserID,
                                                                TabletID, DataSyncDateTime, OrderConfirmFlag, ItemType, PerPoundItemsWeight
                                                          FROM tblOrder 
                                                          INNER JOIN tblCustomer 
                                                          on tblOrder.OrderCustomerNumber=tblCustomer.CustNumber
                                                          ORDER BY OrderDate DESC", connection);

                                                                /*WHERE OrderTrackingNumber IS NOT NULL AND
                                                                OrderDate IS NOT NULL AND
                                                                OrderCustomerNumber IS NOT NULL AND
                                                                CustName IS NOT NULL AND
                                                                CustOfficeAddress1 IS NOT NULL AND
                                                                OrderRouteNumber IS NOT NULL*/
                    // command.CommandType = CommandType.StoredProcedure;
                    //  command.CommandTimeout = 6000;
                    SqlDataAdapter adp = new SqlDataAdapter(command);
                    adp.Fill(ds);
        ...
        public DataSet GetEmail(string userInfo)
        {
            DataSet ds = new DataSet();
            ...
            try
            {
                ...
                {
                    connection.Open();
                    SqlCommand command = new SqlCommand($"select UserID, EmailID FROM tblUserLogin WHERE UserID = \'{userInfo}\' OR EmailID = \'{userInfo}\'", connection);
                    // command.CommandType = CommandType.StoredProcedure;
                    //  command.CommandTimeout = 6000;
                    SqlDataAdapter adp = new SqlDataAdapter(command);
                    adp.Fill(ds);
